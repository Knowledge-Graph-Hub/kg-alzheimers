...
def download(
    ...
    if ingests:
        download_from_yaml(
            yaml_file="src/kg_alzheimers/download.yaml",
            output_dir=".",
            tags=ingests,
        )
    elif all:
        download_from_yaml(yaml_file="src/kg_alzheimers/download.yaml", output_dir=".")
    if write_metadata:
...
def merge(
    ...
    expected_counts = yaml.safe_load(open(f"src/kg_alzheimers/qc_expect.yaml"))
    error = False
