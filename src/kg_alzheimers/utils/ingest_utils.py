...
def get_ingests():
    return yaml.safe_load(pkgutil.get_data("kg_alzheimers", "ingests.yaml"))


def get_ingest(source: str):
    ingests = get_ingests()
    return yaml.load(pkgutil.get_data("kg_alzheimers", ingests[source]["config"]), UniqueIncludeLoader)
...
def ingest_output_exists(source, output_dir):
    ...
    ingest_config = yaml.load(pkgutil.get_data("kg_alzheimers", ingests[source]["config"]), UniqueIncludeLoader)
